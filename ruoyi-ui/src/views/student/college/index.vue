    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['student:college:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['student:college:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['student:college:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['student:college:export']"
        >导出</el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.collId)
    ...
    handleUpdate(row) {
      this.reset();
      const collId = row.collId || this.ids
      getCollege(collId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const collIds = row.collId || this.ids;
      this.$modal.confirm('是否确认删除学院编号为"' + collIds + '"的数据项？').then(function() {
        return delCollege(collIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('student/college/export', {
        ...this.queryParams
